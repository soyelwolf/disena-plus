...
// Shared by every table service — create once, reuse for all integrations.
...
/**
 * Follow @odata.nextLink cursors until every record is retrieved.
 * Use only when all records are genuinely needed (dropdowns, lookups, exports).
 */
 ...
 * @param navigationProperty - The navigation property name, case-sensitive (e.g., 'dpl_Curso')
 * @param entitySetName - The target entity set (e.g., 'dpl_cursos')
...
const formatExpandOption = (opt: ExpandOption): string => {
  ...
  return parts.length > 0 ? `${opt.property}(${parts.join(';')})` : opt.property
}
...
/** Read the current Power Pages portal user injected by the runtime, or null when anonymous. */
export const getPortalUser = (): PortalUser | null => {
  if (typeof window === 'undefined') return null
  const raw = (window as unknown as {
    Microsoft?: { Dynamic365?: { Portal?: { User?: Record<string, unknown> } } }
  }).Microsoft?.Dynamic365?.Portal?.User
  if (!raw?.userName) return null
  return {
    contactId: (raw.contactId ?? raw.userId) as string | undefined,
    userName: raw.userName as string | undefined,
    firstName: raw.firstName as string | undefined,
    lastName: raw.lastName as string | undefined,
    email: (raw.email ?? raw.emailAddress) as string | undefined,
    webRoles: raw.webRoles as PortalUser['webRoles'],
  }
}

export const getCurrentContactId = (): string | undefined => getPortalUser()?.contactId
