...
-- 2. LISTADO_CURSOS_PARA_IA columns that were not migrated.
...
alter table dpl_curso add column if not exists dpl_permitematrizcn boolean not null default false;        -- Permite_Matriz_CN
...
alter table dpl_curso add column if not exists dpl_notif_matrizconrubrica_enviada boolean not null default false; -- NotificacionMatrizConRubricaEnviada
